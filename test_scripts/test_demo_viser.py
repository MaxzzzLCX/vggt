...
def viser_wrapper(
    ...
    colors = images.permute(0, 2, 3, 1).cpu().numpy()  # now (S, H, W, 3)
...
def main():
    ...
    predictions["images"] = images  # Add the images tensor to the predictions dict

    print("Processing model outputs...")
    for key in predictions.keys():
        if key != "images" and isinstance(predictions[key], torch.Tensor):
