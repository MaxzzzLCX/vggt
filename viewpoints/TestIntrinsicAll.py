"""
Autuomate the pipeline of volume estimation using the HeightEstimationIntrinsic.py script.
"""
import argparse
import os
...
import HeightEstimationIntrinsic
...
def main():
    ...
    if args.save_csv:
        # Write CSV header
        result_csv = os.path.join(args.dataset_folder, f'height_estimation_results_{args.start_index}_{args.end_index}.csv')
    ...
    for i, object_folder in enumerate(sorted(list(Path(args.dataset_folder).glob('id-*/')))[args.start_index:args.end_index]):
        ...
        estimated_volume = HeightEstimationIntrinsic.height_estimation(
    ...
    if args.save_npy:
        npz_path = os.path.join(args.dataset_folder, f'height_estimation_results_{args.start_index}_{args.end_index}.npz')
        np.savez_compressed(
            npz_path,
            objects=all_objects,
